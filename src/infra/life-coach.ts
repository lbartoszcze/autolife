...
import {
  applyObjectivePreferenceBias as applyObjectivePreferenceBiasExtractor,
  applyPreferenceDecay as applyPreferenceDecayExtractor,
  estimateAffect as estimateAffectExtractor,
  estimateNeeds as estimateNeedsExtractor,
  inferObjectivesFromInterventionId as inferObjectivesFromInterventionIdExtractor,
  learnPreferencesFromMessages as learnPreferencesFromMessagesExtractor,
  learnPreferencesFromOutcome as learnPreferencesFromOutcomeExtractor,
  resolveObjectives as resolveObjectivesExtractor,
...
function resolveObjectives(
  ...
  return resolveObjectivesExtractor(cfg?.objectives, needs);
}
...
function applyObjectivePreferenceBias(params: {
  ...
  return applyObjectivePreferenceBiasExtractor({
...
function estimateNeeds(messages: TranscriptMessage[]): LifeCoachNeedScores {
  return estimateNeedsExtractor(messages);
}
...
function estimateAffect(
  ...
  return estimateAffectExtractor(messages, needs);
}
...
function learnPreferencesFromMessages(params: {
  ...
  learnPreferencesFromMessagesExtractor({
    state: { preferences: params.state.preferences },
    messages: params.messages,
  });
}

function inferObjectivesFromInterventionId(intervention: string): string[] {
  return inferObjectivesFromInterventionIdExtractor(intervention);
}
...
function learnPreferencesFromOutcome(params: {
  ...
  learnPreferencesFromOutcomeExtractor({
    state: { preferences: params.state.preferences },
